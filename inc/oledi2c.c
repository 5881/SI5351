...
void oled_send_cmd2(uint8_t *cmd, uint16_t n){
	...
	while ((I2C_SR2(OLEDI2C) & I2C_SR2_BUSY))__asm__("nop");
	i2c_send_start(OLEDI2C);
	// Wait for master mode selected
	while (!((I2C_SR1(OLEDI2C) & I2C_SR1_SB)
		& (I2C_SR2(OLEDI2C) & (I2C_SR2_MSL | I2C_SR2_BUSY))));
	i2c_send_7bit_address(OLEDI2C, OLEDADDR, I2C_WRITE);
	// Waiting for address is transferred.
	while (!(I2C_SR1(OLEDI2C) & I2C_SR1_ADDR));
	// Clearing ADDR condition sequence.
	(void)I2C_SR2(OLEDI2C);
    
     //перед каждой командой шлём 0x80 CMD
	
	for (size_t i = 0; i < n; i++) {
		i2c_send_data(OLEDI2C, 0x80);
		while (!(I2C_SR1(OLEDI2C) & (I2C_SR1_BTF)));
		i2c_send_data(OLEDI2C, cmd[i]);
		while (!(I2C_SR1(OLEDI2C) & (I2C_SR1_BTF)));
		}
	i2c_send_stop(OLEDI2C);
	}
